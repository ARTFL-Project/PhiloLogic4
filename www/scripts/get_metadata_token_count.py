...
from philologic.MetadataQuery import corpus_cmp, expand_grouped_query, make_grouped_sql_clause
...
def query_lowlevel(db,param_dict):
    ...
    if clauses:
        query = "SELECT word_count, philo_id FROM toms WHERE "+ " AND ".join("(%s)" % c for c in clauses) + ";"
    else:
...
def get_metadata_token_count(environ,start_response):
    ...
    for label, m in sorted_frequencies[start_hits_done:]:
        ...
        try:
            frequencies[label]['count'] = round(m['count'] / total_count * 10000, 3)
        except:
            count += 1
