...
function left_pos() {
    var prev_obj_pos = $('.prev_page').offset().left + $('.prev_page').width();
    var page_pos = $('.book_page').offset().left;
    ...
    <div class="page_display">
        ...
        <div class="next_obj_wrapper">
            <div class='next_and_read'>
                <span class="next_page" id="${next_page}">&gt;</span>
        ...
        <div class="book_page" id="${current_page}">
            <div class="obj_text" id="${'_'.join([str(j) for j in obj.philo_id])}">${page_text}</div>
        </div>
    </div>
</div>
