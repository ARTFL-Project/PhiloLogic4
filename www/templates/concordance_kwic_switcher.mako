<div id="report_switch" class="btn-group">
    <%
    concordance_script = "%s/reports/concordance_switcher.py?%s" % (config.db_url, q['q_string'].replace('report=kwic', 'report=concordance'))
    kwic_script = "%s/reports/concordance_switcher.py?%s" % (config.db_url, q['q_string'].replace('report=concordance', 'report=kwic'))
    %>
