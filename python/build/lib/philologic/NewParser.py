...
TEI_MetadataXPaths = { "doc" : [(ContentExtractor,"./teiHeader/fileDesc/titleStmt/author","author"),
             ...
             "word": [(ContentExtractor,".","token"),
                      (AttributeExtractor,".@lemma","lemma"),
                      (AttributeExtractor,".@pos","pos"),
                      (AttributeExtractor,".@authority","authority")]
           }

Default_Token_Regexp = ur"([^ \.,;:?!\"\n\r\t\u00B7]+)|([\.;:?!\u00B7])"
    ...
    def feed(self,*event):
        ...
        if e_type == "text":
            ...
            # Should push a sentence by default here, if we're in a new para/div/doc.  sent byte ordering is not quite right.
                tokens = re.finditer(self.token_regexp,content,re.U) # should put in a nicer tokenizer.
                for t in tokens:
                    if t.group(1):
            ...
            if self.stack:
                ...
                for xpath,ohco_type in self.map.items():
                    ...
                    if current_element in self.root.findall(xpath):
                        if ohco_type == "word":
                            if "token" in self.v["word"].attrib:
#                                 pass
                                self.v["word"].name = self.v["word"]["token"] 
                        self.in_word_tag = False
