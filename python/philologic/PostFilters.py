    ...
    def inner_make_sql_table(loader_obj):
        ...
        for index in indices:
            try:
                ...
                index = ','.join(index)
                c.execute('create index if not exists %s on %s (%s)' %
...
def normalized_word_frequencies(loader_obj):
    ...
    for line in open(frequencies + '/word_frequencies'):
        ...
        norm_word = [i
                     for i in unicodedata.normalize("NFKD", norm_word)
...
def normalized_metadata_frequencies(loader_obj):
    ...
    for field in loader_obj.metadata_fields:
        try:
            output = open(
                frequencies + "/normalized_" + field + "_frequencies", "w")
...
DefaultPostFilters = [word_frequencies, normalized_word_frequencies,
                      metadata_frequencies, normalized_metadata_frequencies]
