...
class Config(object):
    ...
    def __init__(self, filename, defaults):
        ...
        print >> sys.stderr, "filename", repr(self.filename), repr(filename)
        self.defaults = defaults
        print >> sys.stderr, "defaults", repr(self.defaults), repr(defaults)
        self.data = {}
        ...
        if (self.filename):
          ...
          file_data = {}
          execfile(self.filename,globals(),file_data)
    ...
    def write(filename,header=""):
        written_keys = []
        fh = open(filename,"w")
        fh.write(header)
        for key,value in self.sorted_defaults:
          fh.write(value["comments"])
          fh.write("\n")
          fh.write("\n%s = %s\n" % (key,repr(data[key])) )
...
def WebConfig():
        return Config(web_config_path,web_config_defaults)
 
 
def DBConfig():
        return Config(db_config_path,db_config_defaults)
