...
cd ..;
sudo mkdir -p /var/lib/philologic4.6/web_app/
sudo rm -rf /var/lib/philologic4.6/web_app/*
sudo cp -R www/* /var/lib/philologic4.6/web_app/
sudo cp www/.htaccess  /var/lib/philologic4.6/web_app/
...
if [ ! -f /etc/philologic/philologic4.6.cfg ]
    then
        ...
        echo "$db_url" | sed "s/^ *//g" | sudo tee /etc/philologic/philologic4.6.cfg > /dev/null
        ...
        echo "$url_root" | sed "s/^ *//g" | sudo tee -a /etc/philologic/philologic4.6.cfg > /dev/null
        web_app_dir="## This should be set to the location of the PhiloLogic5 www directory
        web_app_dir = '/var/lib/philologic5/web_app/'"
        echo "$web_app_dir" | sed "s/^ *//g" | sudo tee -a /etc/philologic/philologic4.6.cfg > /dev/null
        ...
        echo "$theme" | sed "s/^ *//g" | sudo tee -a /etc/philologic/philologic4.6.cfg > /dev/null
else
    echo "\n## WARNING ##"
    echo "/etc/philologic/philologic4.6.cfg already exists"
